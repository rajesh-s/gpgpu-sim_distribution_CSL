# Changes from stable GPGPU-sim branch to get it working on CSL Machines

Tested on CUDA9.1/11.5 Ubuntu 20.04

1.  Update LD_LIBRARY_PATH, PATH, CUDA_INSTALL_PATH, CUDA_HOME, and CUDA_PATH
	...
	export LD_LIBRARY_PATH=/s/cuda-9.1/amd64_ubu20/lib64:/s/mpfr-3.1.6/amd64_ubu20/lib:/s/gcc-5.4.0/amd64_ubu20/lib64:/s/gcc-5.4.0/amd64_ubu20/lib:$LD_LIBRARY_PATH
	export PATH=/s/cuda-9.1/amd64_ubu20/bin:/s/gcc-5.4.0/amd64_ubu20/bin:$PATH
	```

        ```bash
        # CUDA 11.5 
        export CUDA_HOME=/u/r/a/rajesh/cuda
        export CUDA_PATH=$CUDA_HOME
        export CUDA_INSTALL_PATH=$CUDA_HOME
        export LD_LIBRARY_PATH=/u/r/a/rajesh/cuda/lib64:/s/mpfr-3.1.6/amd64_ubu20/lib:/s/gcc-5.4.0/amd64_ubu20/lib64:/s/gcc-5.4.0/amd64_ubu20/lib:$LD_LIBRARY_PATH
        export PATH=/u/r/a/rajesh/cuda/bin:/s/gcc-5.4.0/amd64_ubu20/bin:$PATH
        ```
	

2.  Run "source setup_environment release" in the top-level GPGPU-Sim folder

3.  Make GPGPU-Sim: ```make -j5```

*** For step 3, several updates to the Makefiles are needed in GPGPU-Sim from the stable branch to add the std=c++11 flags to the Makefiles. Thanks to Prof. Sinclair for the patchfile (Makefile.patch) containing these changes.
